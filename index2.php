...
$f3->route('GET /users',function($f3){
	...
	try{
		...
		$query = "SELECT * FROM `users` u left outer join education e on u.user_name = e.username left outer join work w on u.user_name = w.user_name left outer join location l on u.user_name = l.user_name where l.location_id = (select max(location_id) from location where u.user_name = location.user_name)";
		if(!empty($gender)){
