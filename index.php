...
$f3->route('POST /createChat',function($f3){
    ...
    try{
		...
		$toUser = $users->findone(array('user_name = ? or email = ?',$to,$to));
        if(empty($toUser)){
            $response['status'] = 412;
...
$f3->route('GET /recieve',function($f3){
    ...
    try{
		...
		$chat = $chats->find(["to_user = ? and status = 'recieved'",$user->user_name]);
        $messages = [];
    ...
    try{
        ...
        if(empty($user)){
            ...
            if(!empty($location)){
				...
				$response['data'] = arrayMerge($response['data'],$location->cast(),true);
            }
            $educations = new DB\SQL\Mapper($db,'education');
            $education = $educations->findone(['username = ?',$user->user_name]);
            if(!empty($education)){ 
                //$response['data']['education'] = $education->cast();
				$response['data'] = arrayMerge($response['data'],$education->cast(),true);
            }
            $works = new DB\SQL\Mapper($db,'work');
            $work = $works->findone(['user_name = ?',$user->user_name]);
            if(!empty($work)){
                //$response['data']['work'] = $work->cast();
				$response['data'] = arrayMerge($response['data'],$work->cast(),true);
            }
            unset($response['data']['password']);
            echo json_encode($response);
            return;
...
$f3->route('GET /usersbydistance',function($f3){
	...
	try{
		...
		if(count($usersList) > 0){
			...
			$response['user'] = arrayMerge($response['user'],$location->cast(),true);
...
$f3->route('GET /users',function($f3){
	...
	try{
		...
		$gender = clean($f3, 'GET.genders');
		$location = clean($f3, 'GET.locations');
		$passout = clean($f3, 'GET.passouts');
		$courses = clean($f3, 'GET.courses');
		$universities = clean($f3, 'GET.universities');
		$companies = clean($f3, 'GET.companies');
		$designations = clean($f3, 'GET.designations');
		$distance = clean($f3, 'GET.distance');
		if(!empty($distance)){
			if(is_numeric($distance)){
				$locations = new DB\SQL\Mapper($db,'location');
				$lcn = $locations->findone(['user_name = ?',$user->user_name],['order' => 'location_id DESC']);
				if(empty($lcn)){
					$response['status'] = 412;
					$response['message'] = 'Please update users current location to list users based on distance';
					echo json_encode($response);
					return; 
				}
				if(!checkLatitude($lcn->latitude) && !checkLongitude($lcn->longitude)){
					$response['status'] = 412;
					$response['message'] = 'Location details are wrong';
					echo json_encode($response);
					return; 
		...
		$query = "SELECT * FROM `users` u left outer join education e on u.user_name = e.username left outer join work w on u.user_name = w.user_name left outer join location l on u.user_name = l.user_name where l.location_id = (select max(location_id) from location where u.user_name = location.user_name) and u.user_name <> '$user->user_name'";
		if(!empty($gender)){
		...
		if(!empty($location)){
			if(gettype($location) === 'array'){
				...
				$query .= " and l.location in " . $location;
			}else{
				$query .= " and l.location like '%" . $location . "%'";
			}
		}
		if(!empty($passout)){
			if(gettype($passout) === 'array'){
				$passout = "('" . implode($passout,"','") . "')";
				$query .= " and passout in " . $passout;
			}else{
				$query .= " and passout like '%" . $passout . "%'";
			}
		}
		if(!empty($courses)){
			if(gettype($courses) === 'array'){
				$courses = "('" . implode($courses,"','") . "')";
				$query .= " and course in " . $courses;
			}else{
				$query .= " and course like '%" . $courses . "%'";
			}
		}
		if(!empty($universities)){
			if(gettype($universities) === 'array'){
				$universities = "('" . implode($universities,"','") . "')";
				$query .= " and course in " . $universities;
			}else{
				$query .= " and course like '%" . $universities . "%'";
			}
		}
		if(!empty($companies)){
			if(gettype($companies) === 'array'){
				$universities = "('" . implode($companies,"','") . "')";
				$query .= " and company in " . $companies;
			}else{
				$query .= " and company like '%" . $companies . "%'";
			}
		}
